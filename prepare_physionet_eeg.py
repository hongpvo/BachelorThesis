"""
This function use to read time-series data *PSG.edf and its label *Hypnogram.edf
and later save it as numpy *.npz files for later use

Besides, it also pre-process the data as:

1. Remove movement and unknown stages
2. Select on sleep periods (max 30min W stage before and after sleep phase to prevent dominated W stage data)
3. Split the data to 30-s epoch

To preprare the eeg data please run:
!python prepare_physionet.py --data_dir YOUR_DATA_FOLDER --output_dir YOUR_OUTPUT_FOLDER --select_ch 'EEG Fpz-Cz'
!python prepare_physionet.py --data_dir YOUR_DATA_FOLDER --output_dir YOUR_OUTPUT_FOLDER --select_ch 'EEG Fpz-Cz'

This pre-processing is based on DeepSleepNet paper with modification.
